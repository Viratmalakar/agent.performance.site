...
net_login_sec = (login_sec - break_sec).clip(lower=0)

final["Total Break"] = break_sec.map(stime)
final["Total Meeting"] = meet_sec.map(stime)
final["Total Net Login"] = net_login_sec.map(stime)
...
# 👉 ONLY TOTAL CALL RED CONDITION
final["__red_call"] = (net_login_sec >= 8*3600) & (final["Total Call"] < 100)
